...
public class AsyncPoolConfig implements AsyncConfigurer {
    ...
    public Executor getAsyncExecutor() {
        ...
        executor.setThreadNamePrefix("Jason-Async-");   // 这个非常重要
        executor.setWaitForTasksToCompleteOnShutdown(true); // 等待所有任务结果候再关闭线程池
        executor.setAwaitTerminationSeconds(60);
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy()); // 定义拒绝策略
        executor.initialize(); // 初始化线程池, 初始化 core 线程
